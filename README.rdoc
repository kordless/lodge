...
Lodge forwards TCP based syslog data to the local machine on port 514.  By default, most syslog servers will not be configured to take TCP streams on port 514. You'll probably need fiddle with your syslog server a bit to get it working.  
...
In thory, Node.js can do UDP datagrams which also means Lodge could probably be reworked slightly to support sending to an UDP based syslog server listening on port 514.  Food for thought.
